...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    for{
      n <- neighbors
      if explored.contains(n._1) == false
    } yield n
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, movesL) #:: rest => {
       val nN = newNeighborsOnly(neighborsWithHistory(block, movesL), explored + block)
       nN ++ from(rest ++ nN, explored + block)
    }
  }
